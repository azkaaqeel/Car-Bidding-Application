...
const CarDetails = () => {
    ...
    const placebid = async () => {
        try {
            const response = axios.post('http://localhost:5000/api/bids', {
                buyer_id: userId, // Buyer ID from state/input
                bid_amount: document.getElementById('bid').value, // Bid amount from state/input
                car_id: car_id, // Send car ID
            });
            console.log(response)
            alert("Bid placed successfully! Bid ID");
